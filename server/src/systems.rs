...
fn flush_pending_connections(
    ...
    for owner in pending.0.drain(..) {
        let Ok(remote_id) = query.get(owner) else { continue };
        let char_key = if counter.0 % 2 == 0 { "comini" } else { "kaps" };
        counter.0 += 1;
        spawn_player(owner, remote_id.0, char_key, &registry, &char_assets, &mut commands);
    }
}
...
fn check_player_death(
    ...
    for (entity, health, char_type, controlled_by) in &players {
        if health.is_dead() {
            let Ok(remote_id) = owners.get(controlled_by.owner) else { continue };
